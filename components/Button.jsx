import React from 'react';

const iconUrl = {
    hamburger: 'data:image/png;base64,',
    close: 'data:image/svg+xml;base64,'
};

const Button = ({ variant = '', style, onClick, className = '' }) => {

    return(
        <button 
        variant = {variant}
        style={style} 
        onClick={onClick} 
        className={`${className || ''} ${variant}`}
        >
            <img src={iconUrl[variant] || 'data:image/png;base64,'} alt={`${variant} icon`} />
       </button>
    );
};

export default Button;